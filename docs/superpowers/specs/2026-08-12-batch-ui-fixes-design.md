...
## ⑦ Q&A 이력 저장 (관례 학습 데이터 심기)

- 저장 시 세션 messages에서 (AI 질문 → 직후 사용자 답) 쌍 추출 ("건너뛰기" 답 제외).
- `RecordIn.qa: list[{question, answer}] = []` → `save_record`가 본문 끝에
  `## 재질문` 섹션으로 기록 (frontmatter 아님 — 위키 편찬 LLM이 자연히 읽도록).
- 소비 기능(관례 편찬·프롬프트 주입)은 파일럿 후 별도 — 지금은 데이터만 쌓는다.
...
파인튜닝·Hermes류 상주 에이전트, 관례 자동 편찬(⑦의 소비측), needs_review 자동 해제,
세션 메뉴의 아카이브/내보내기, 제목 일괄 재생성(기존 기록).
