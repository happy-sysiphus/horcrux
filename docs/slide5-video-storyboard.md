...
1. **프레임 연출 (가장 효과 큼)** — 녹화본을 그대로 꽉 채우지 말고: 배경에 은은한
   ...
   순간적으로 "SaaS 광고 룩"이 된다. (편집 → 배경 캔버스 / 클립에 둥근 모서리·그림자)
2. **줌 키프레임** — 클립의 배율·위치에 키프레임 2개: 행동 직전(100%) → 행동 지점
   (130~150%, 해당 영역 중앙). 클릭·입력마다 카메라가 따라다니는 느낌. 컷당 2회 이내.
3. **스포트라이트** — 구조 패널이 채워지는 순간: 클립 복제 → 위 클립에 원형/사각
   마스크(패널 영역) → 아래 클립 밝기 -40%. 주변이 어두워지고 패널만 빛난다.
4. **클릭 리플** — 칩 클릭·저장 클릭 순간에 스티커 검색 "click circle" 또는 원형 도형을
   0.3초 스케일 팝으로. 클릭이 어디서 났는지 시선이 따라간다.
5. **라벨 슬라이드인** — 빨간 텍스트 + 꺾인 지시선(스티커 "arrow" 검색), 등장 애니메이션
   0.3초 슬라이드인 + 사라질 때 페이드. 화면당 최대 2개 유지.
6. **속도 램프** — 타이핑·로딩 구간 2~3배속, 재질문 답변·구조 패널 완성 순간은 정속(또는
   0.8배속으로 살짝 눌러 강조). 클립 우클릭 → 곡선 변속이 자연스럽다.

시간 아끼는 지름길: 녹화 자체를 **클릭 자동 줌 녹화 툴**로 하면 2번이 공짜로 된다
(클릭 위치로 카메라가 자동 줌·팬). 크롬 확장 Cursorful 류를 쓰면 되고, 그 결과물에
캡컷에서 1·5·6번만 얹어도 충분히 간지난다.
