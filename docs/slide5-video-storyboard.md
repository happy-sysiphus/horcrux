...
| 2 | 4~7s | 폰 화면으로 쭉 줌인 → 서비스 화면 매치컷 | 실사 + 편집 전환 |
| 3 | 7~12s | 실험 로그가 입력되는 앱 화면 | 화면 녹화 (모바일 뷰) |
| 4 | 12~19s | 파싱 로딩 → 재질문 → 저장 | 화면 녹화 (모바일 뷰) |
...
## 컷 2 — 매치컷: 폰 → 서비스 화면 (4~7초)

- 컷 1의 마지막 1초에서 폰 화면을 향해 **디지털 줌인**(CapCut 키프레임 확대).
- 화면이 프레임을 거의 채우는 순간 → **컷 3의 앱 녹화 화면으로 하드컷**(또는
  0.2초 화이트 플래시). 줌인 속도가 빠를수록 전환이 자연스럽다.
- 라벨 없음 — 전환 컷은 깨끗하게.
...
- 크롬 → F12 → 기기 툴바(Ctrl+Shift+M) → **iPhone 12 Pro (390×844)** — 폰에서 이어지는
  매치컷이니 모바일 뷰로 녹화한다. 데모 볼트는 기록 11건 시딩돼 있어 화면이 풍성하다.
