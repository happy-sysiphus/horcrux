...
일일 LLM 호출 상한은 **서비스 운영자만** 정한다. 연구실 관리자는 설정 화면에서
오늘 사용량만 볼 수 있고 상한은 못 바꾼다 — 중앙 API 키 비용이 연구실 손에 달리면 안 되기 때문이다.

바꾸는 곳: Supabase → **Table Editor → `labs`** → 해당 연구실 행의
`daily_llm_limit` 셀을 클릭해 숫자 입력 → Enter. 재배포·재시작 없이 다음 호출부터 적용된다.
새로 만들어지는 연구실은 200으로 시작한다(기본값을 바꾸려면 `db/schema.sql`의
`default 200`을 고치고 `alter table labs alter column daily_llm_limit set default <값>;` 실행).

**1회로 세어지는 것**: 로그 파싱, 기록 저장, 문제 질문. 조회·참고문헌 편집은 세지 않는다.
재질문 루프에서 다시 파싱할 때마다 또 1회이므로, 200은 대략 하루 40~50건 기록에 해당한다.
카운트는 서버 기준 날짜가 바뀔 때 초기화된다(Railway는 UTC이므로 한국 시간 오전 9시).

상한을 넘기면 사용자 화면에 "오늘 사용량 한도를 초과했습니다"가 뜬다.
